...
which dispatches to cuDNN flash attention on supported GPUs.
...
def _normalize_config(config: dict[str, Any]) -> dict[str, Any]:
    """Validate and normalize an FA4 benchmark config."""
...
def _config_from_env() -> dict[str, Any]:
    """Read FA4 config from CUDA_EXEC_CONFIG_JSON environment variable."""
...
class Model(nn.Module):
    """Flash Attention forward reference.
    ...
    def __init__(self):
        super().__init__()
        self._causal = False  # Set per-call from config; default non-causal
...
def main() -> int:
    ...
    # Only serialize a small slice of the output to avoid huge JSON payloads

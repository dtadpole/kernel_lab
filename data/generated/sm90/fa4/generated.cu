    ...
    const __grid_constant__ CUtensorMap tma_O)
{
    ...
    /* ---- PROLOGUE: QK[0] ---- */
    {
        ...
        /* Softmax (first block) — causal: tree reduction, noncausal: interleaved h0/h1 */
        if constexpr (IS_CAUSAL) {
            ...
            /* Noncausal: interleaved h0/h1 for better ILP */
            float lmax0 = S_acc[0], lmax1 = S_acc[2];
            #pragma unroll
            for (int p4 = 0; p4 < 16; p4++) {
                lmax0 = fmaxf(lmax0, S_acc[(p4<<2)|0]);
                lmax1 = fmaxf(lmax1, S_acc[(p4<<2)|2]);
                lmax0 = fmaxf(lmax0, S_acc[(p4<<2)|1]);
                lmax1 = fmaxf(lmax1, S_acc[(p4<<2)|3]);
            }
            ...
            rowmax[0] = lmax0; rowmax[1] = lmax1;
            float neg0 = -lmax0 * softmax_scale_log2, neg1 = -lmax1 * softmax_scale_log2;
            float lsum0 = 0, lsum1 = 0;
            #pragma unroll
            for (int p4 = 0; p4 < 16; p4++) {
                float e;
                e = fast_exp2f(fmaf(S_acc[(p4<<2)|0], softmax_scale_log2, neg0));
                S_acc[(p4<<2)|0] = e; lsum0 += e;
                e = fast_exp2f(fmaf(S_acc[(p4<<2)|2], softmax_scale_log2, neg1));
                S_acc[(p4<<2)|2] = e; lsum1 += e;
                e = fast_exp2f(fmaf(S_acc[(p4<<2)|1], softmax_scale_log2, neg0));
                S_acc[(p4<<2)|1] = e; lsum0 += e;
                e = fast_exp2f(fmaf(S_acc[(p4<<2)|3], softmax_scale_log2, neg1));
                S_acc[(p4<<2)|3] = e; lsum1 += e;
            }
            rowsumexp[0] = lsum0; rowsumexp[1] = lsum1;
        }
    ...
    for (int kv_id = 1; kv_id < max_kv_iter; kv_id++) {
        ...
        if constexpr (IS_CAUSAL) {
            ...
            /* Noncausal: interleaved h0/h1 for better ILP, in-place on S_acc */
            float lmax0 = S_acc[0], lmax1 = S_acc[2];
            #pragma unroll
            for (int p4 = 0; p4 < 16; p4++) {
                lmax0 = fmaxf(lmax0, S_acc[(p4<<2)|0]);
            ...
            lmax0 = fmaxf(lmax0, __shfl_xor_sync(0xFFFFFFFF, lmax0, 2));
            lmax1 = fmaxf(lmax1, __shfl_xor_sync(0xFFFFFFFF, lmax1, 2));
            float new_max0 = fmaxf(lmax0, rowmax[0]);
            float new_max1 = fmaxf(lmax1, rowmax[1]);
            o_rescale[0] = fast_exp2f((rowmax[0] - new_max0) * softmax_scale_log2);
            o_rescale[1] = fast_exp2f((rowmax[1] - new_max1) * softmax_scale_log2);
            rowmax[0] = new_max0; rowmax[1] = new_max1;
            float neg0 = -new_max0 * softmax_scale_log2, neg1 = -new_max1 * softmax_scale_log2;
            float lsum0 = 0, lsum1 = 0;
            #pragma unroll
            for (int p4 = 0; p4 < 16; p4++) {
                float e;
            ...
            rowsumexp[0] = rowsumexp[0] * o_rescale[0] + lsum0;
            rowsumexp[1] = rowsumexp[1] * o_rescale[1] + lsum1;
        }
        ...
        /* Rescale: causal=conditional skip, noncausal=branchless (AVO paper) */
        #pragma unroll
        for (int half = 0; half < 2; half++) {

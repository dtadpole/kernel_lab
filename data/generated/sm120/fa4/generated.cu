 ...
 * All-compute architecture: all threads participate in both loads and MMA.
 *
 * Layout:
 *   Q, K, V, O  : [B, S, H, D]  (row-major __nv_bfloat16)
 *
 * Constants: BLOCK_Q=128, BLOCK_KV=128, DIM=128, 8 warps (256 threads).
 *
 * All-compute architecture (no warp specialization):
 *   All 256 threads participate in cp.async loads AND mma.sync compute.
 ...
 *   - WARP_Q = 128/8 = 16 (one MMA_M tile per warp)
 *   - S_local[16][4] = 64 floats
 *   - O_rmem[1][16][4] = 64 floats
 *   - P_rmem[1][8][4] = 32 uint32
 *
 * kernel_run contract:
...
/* ======================================================================
 *  All-compute Flash Attention kernel
 *
...
__launch_bounds__(256, 1)  /* 1 block/SM: let compiler use full 255 regs */
__global__
    ...
    int is_causal)
{
    ...
    /* O accumulator, rowmax, rowsumexp — WARP_Q/MMA_M = 1, so no mma_id_q loop */
    float O_rmem[DIM / MMA_N][4] = {};
    float rowmax[2] = {-FLT_MAX, -FLT_MAX};
    float rowsumexp[2] = {};

    /* ---- Prologue: load K[0] ---- */
    global_to_shared_swizzle<BLOCK_KV, DIM, TB_SIZE>(
        K_smem, K_base, seq_stride, tid);
    asm volatile("cp.async.commit_group;");
    asm volatile("cp.async.wait_all;");
    __syncthreads();
    ...
    for (int kv_id = 0; kv_id < max_kv_iter; kv_id++) {
        /* K is already in K_smem (from prologue or previous iteration) */

        /* Start V load (overlaps with QK compute) */
        const nv_bfloat16 *V_ptr = V_base + kv_id * BLOCK_KV * seq_stride;
        global_to_shared_swizzle<BLOCK_KV, DIM, TB_SIZE>(
        ...
        /* Prefetch Q for d=0 */
        uint32_t Q_cur[4];
        ldmatrix_x4(Q_cur, Q_smem_thread);

        #pragma unroll
        for (int mma_id_d = 0; mma_id_d < DIM / MMA_K; mma_id_d++) {
            /* Prefetch Q for next d-step */
            uint32_t Q_next[4];
        ...
        if (is_causal) {
            ...
            const int q_first = q_start + warp_id * WARP_Q;
        ...
        float rescale[2];
        rescale[0] = exp2f(rowmax[0] - this_rowmax[0]);
        rescale[1] = exp2f(rowmax[1] - this_rowmax[1]);
        #pragma unroll
        ...
        uint32_t P_rmem[BLOCK_KV / MMA_K][4];
        #pragma unroll
        for (int mma_id_kv = 0; mma_id_kv < BLOCK_KV / MMA_N; mma_id_kv++) {
            float *regs = S_local[mma_id_kv];
            regs[0] = exp2f(regs[0] - rowmax[0]);
            regs[1] = exp2f(regs[1] - rowmax[0]);
            regs[2] = exp2f(regs[2] - rowmax[1]);
            regs[3] = exp2f(regs[3] - rowmax[1]);
            ...
            nv_bfloat162 *this_P =
                reinterpret_cast<nv_bfloat162 *>(P_rmem[mma_id_kv / 2]);
        ...
        rowsumexp[0] = rowsumexp[0] * rescale[0] + this_rowsumexp[0];
        rowsumexp[1] = rowsumexp[1] * rescale[1] + this_rowsumexp[1];

        /* ---- Wait for V load, then start next K load ---- */
        asm volatile("cp.async.wait_all;");
        __syncthreads();

        /* Start loading K[kv_id+1] during PV compute */
        if (kv_id + 1 < max_kv_iter) {
            const nv_bfloat16 *K_ptr = K_base + (kv_id + 1) * BLOCK_KV * seq_stride;
            global_to_shared_swizzle<BLOCK_KV, DIM, TB_SIZE>(
        ...
        /* ---- O += P @ V ---- */
        {
            ...
            for (int mma_id_kv = 0; mma_id_kv < BLOCK_KV / MMA_K; mma_id_kv++) {
                ...
                for (int mma_id_d = 0; mma_id_d < DIM / MMA_N; mma_id_d++) {
                    ...
                    mma_m16n8k16(P_rmem[mma_id_kv], V_cur, O_rmem[mma_id_d]);
        ...
        /* Wait for next K load to finish before next iteration */
        if (kv_id + 1 < max_kv_iter) {
            asm volatile("cp.async.wait_all;");
            __syncthreads();
        }
    ...
    #pragma unroll
    for (int mma_id_d = 0; mma_id_d < DIM / MMA_N; mma_id_d++) {
        const int row = warp_id * WARP_Q + (lane_id / 4);
    ...
    int n, cudaStream_t stream)
{
...

    {
        ...
        const int TB_SIZE   = 256;  /* 8 warps, all compute */
        ...
        int smem_k = BLOCK_KV * DIM_CONST * (int)sizeof(nv_bfloat16);

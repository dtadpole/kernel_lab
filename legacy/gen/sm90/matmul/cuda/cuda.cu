/*
 * BF16 matmul for SM90 — WGMMA + TMA, 3-WG warp-specialized (v14).
 *
 * v14 vs v13:
 *   - STATIC tile scheduling: blockIdx.x + k*gridDim.x (no atomicAdd)
 *   - Removes tile_counter, mbar_tready, tile_info from SMEM
 *   - Producer and consumers compute tile coords independently (no tile_info signal)
 *   - Eliminates ~2048 atomicAdd L2 serializations at 8192x8192
 *   - Simpler synchronization: only mbar_full + mbar_empty remain
 *
 ...
 *
 * CTA tile 128x256, TILE_K=64, 4-stage pipeline.
 * B loaded MN-major via split TMA: four 64-col loads with 128B swizzle.
 * WGMMA m64n64k16 with trans-b=1.
 * Persistent barriers: phase tracking across local tiles, no re-init.
 */
#include <cuda_bf16.h>
#include <cuda.h>
#include <dlfcn.h>
 ...
 * Works for both K-major A (trans=0) and MN-major B quarters (trans=1).
 * LBO=1 (16 bytes), SBO=sbo (1024 bytes for tile_inner=64), swizzle=128B.
 */
__device__ __forceinline__
 ...
 * Stores paired BF16 values using bfloat162 for 32-bit transactions.
 * ========================================================================= */
__device__ __forceinline__
void store_acc_n64(__nv_bfloat16* C, float (&acc)[32],
    ...
    const int row_base = ctaRow + warp * 16 + lane / 4;
    const int col_base = ctaCol + (lane % 4) * 2;
    #pragma unroll
    for (int p = 0; p < 8; p++) {
        const int col = col_base + p * 8;
        const int row0 = row_base, row8 = row_base + 8;
        if (row0 < M && col + 1 < N) {
...
/* =========================================================================
 * Tile coordinate helper — GROUP_M=12 swizzle, computed independently
 * by both producer and consumer from the global tile index.
 * ========================================================================= */
__device__ __forceinline__
...
/* =========================================================================
 * Main kernel — 3-WG warp-specialized, static tile scheduling (v14)
 *
 * WG0 (producer): TMA loads A + B-quarters per stage.
 * WG1 (consumer0): WGMMA on M-rows 0-63, all N-quarters.
 * WG2 (consumer1): WGMMA on M-rows 64-127, all N-quarters.
 *
 * Tile assignment: tile = blockIdx.x + local_k * gridDim.x  (no atomicAdd)
 * Both producer and consumers compute ctaRow/ctaCol independently.
 *
 * Synchronization:
    ...
    int grid_m, int grid_n, int total_tiles)
{
    ...
    if (wg_id == 0) {
        /* =============================================================
         * PRODUCER — static tile loop (no atomicAdd)
         * tid==0 does all TMA work; threads 1-127 sleep.
         * ============================================================= */
        asm volatile("setmaxnreg.dec.sync.aligned.u32 24;\n");

        if (tid != 0) {
            /* Non-producer threads have no work — exit immediately.
             * Warp divergence: threads 1-31 return here; thread 0 continues.
             * After all of threads 1-31 retire, warp 0 runs with only thread 0 active. */
            return;
        }
        ...
        for (int local_k = 0; ; local_k++) {
            ...
            /* Wait for consumers to free last stage of previous tile */
            if (local_k > 0) {
                const int prev_phase_base = (local_k - 1) * phases_per_tile;
                const int last_stage = (numK - 1) % STAGES;
                const int last_phase = (prev_phase_base + (numK - 1) / STAGES) & 1;
        ...
        /* =============================================================
         * CONSUMER — static tile loop (mirrors producer)
         * ============================================================= */
    ...
    int n, cudaStream_t stream)
{
     ...
     * A: K-major (K contiguous), box=[TILE_K, TILE_M], L2_128B promotion.
     * B: MN-major (N contiguous), box=[TILE_NQ=64, TILE_K], L2_128B promotion.
     */
    CUtensorMap tma_A, tma_B;
    {
        ...
        CUresult r = s_encodeTiled(&tma_A, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 2,
            ...
            CU_TENSOR_MAP_L2_PROMOTION_L2_128B,   /* v13: was NONE */
    ...
    /* Static scheduling: 1 block per SM, no atomicAdd */
    int num_sms = 0;
